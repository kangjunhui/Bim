...
function popupList(body, sets) {
    sets.forEach(function (set) {
        ...
        set.PropertyList.forEach(function (prop) {
            var proptr = `<tr><td>${prop.Name}</td><td class="with-form-control"><input type="text" data-name=${prop.Name} data-propset=${set.Name} class="form-control" value=${prop.NominalValue} /></td></tr>`
            body.append(proptr)
        })
    })
    regEnterEvent()
